...
a.addEventListener("submit", async(event) => {
    ...
    try{
        const a_1 = await fetch("http://127.0.0.1:8000/create_user", { // функция fetch отправляет асинхронные http-запросы, возвращая объект response
            method: "POST",
...
b.addEventListener("submit", async(event) => {
    ...
    try{
        const b_1 = await fetch(`http://127.0.0.1:8000/get_user/${name}`, {
            method: "GET",
...
c.addEventListener("click", async(event) => {
    ...
    try{
        const c_1 = await fetch("http://127.0.0.1:8000/get_users", {
            method: "GET",
            headers: {"Content-Type": "application/json"},
        });
